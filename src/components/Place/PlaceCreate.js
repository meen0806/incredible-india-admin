...
import { Create, SimpleForm, TextInput, ReferenceInput, SelectInput, ArrayInput, SimpleFormIterator, NumberInput, ObjectInput, useGetList, FileInput, FileField, Edit } from 'react-admin';
...
return(
  <Create>
    <SimpleForm>
      ...
      <SelectInput source="location_id" label="Location" choices={filteredLocation.map((location)=>({
        id:location.id,
        name : location.name
      }))}
      optionText="name"
    ...
    <SelectInput source="category_id" label="Category" choices={categories?.map((category)=>({
        id:category.id,
        name:category.name
      }))}
      optionText="name"
